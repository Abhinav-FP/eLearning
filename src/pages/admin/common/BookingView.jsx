...
const BookingView = ({ data, status }) => {
    ...
    return (
        <>
            ...
            {showPopup && (
                <Popup isOpen={showPopup} onClose={() => setShowPopup(false)} size="max-w-[1000px]">
                    <div className="space-y-6">
                        ...
                        <InfoCard icon={<FaBook className="text-red-400" />} title="Lesson">
                            <div className="space-y-2 text-left">
                                ...
                                <div className="flex items-center gap-2">
                                    <h2 className="text-xs font-bold text-gray-800 capitalize">Name: {data.LessonId?.title}</h2>
                                </div>
                        ...
                        <InfoCard icon={<FaMoneyBill className="text-red-400" />} title="Payment">
                            <div className="space-y-2 text-left">
                                {/* Total Amount */}
                                {status != "teacher" && (
                                    <div>
                        ...
                        <InfoCard icon={<FaCalendarAlt className="text-red-400" />} title="Booking">
                            <div className="space-y-2 text-left">
                                <div className="space-y-1 text-left">
                                    <p>
                                        <span className="font-semibold">Lesson:</span>
                                        <span className="ml-2 text-gray-800 capitalize">{data.LessonId?.title}</span>
                                    </p>
                                    <p>
                                        <span className="font-semibold">Start:</span>
                                        <span className="ml-2 px-2 py-0.5   rounded-full text-xs font-medium">
                                            {moment(data.startDateTime).format('MMMM D, YYYY  h:mm A')}
                                        </span>
                                    </p>
                                    <p>
                                ...
                                {/* Status pills */}
                                <div className="flex flex-wrap gap-2 mt-2">
                                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${data.lessonCompletedStudent && data.lessonCompletedTeacher
                                        ? 'bg-green-100 text-green-700'
                                        : 'bg-yellow-100 text-yellow-700'}`}>
                                        {data.lessonCompletedStudent && data.lessonCompletedTeacher ? 'Completed' : 'In Progress'}
                                    </span>


                                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${data.rescheduled
                                        ? 'bg-purple-100 text-purple-700'
                                        : 'bg-gray-100 text-gray-600'}`}>
                                        {data.rescheduled ? 'Rescheduled' : 'No Rescheduled'}
                                    </span>

                                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${data.payoutDoneAt
                                        ? 'bg-green-50 text-green-600'
                                        : 'bg-yellow-50 text-yellow-700'}`}>
                                        {data.payoutDoneAt ? `Payout Done at ${moment(data.payoutDoneAt).format('MMMM D, YYYY  h:mm A')}` : data.payoutCreationDate ? ` Payout  Requested at ${moment(data.payoutCreationDate).format('MMMM D, YYYY  h:mm A')}` : "Payout Pending"}
                                    </span>
                                </div>
                            </div>
                        </InfoCard>
                                                    ...
                                                    <div className="flex justify-between items-center w-full mb-1">
                                                        ...
                                                        <p>
                                                            <a
                                                                ...
                                                                className="text-green-600  text-md flex gap-1 items-center"
                                                            >
                                                                <MdDownload />
                                                                Download
                                                            </a>
                                    ...
                                    <div className="border rounded-lg p-4  space-y-3">
                                        {chatMessages?.length === 0 ? (
                                            ...
                                            chatMessages?.map((chat, index) => {
                                                ...
                                                return (
                                                    <div key={index} className={`flex flex-col ${isTeacher ? "items-start" : "items-end"} mb-4`}>
