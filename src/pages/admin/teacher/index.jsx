 ...
 const [teacherData, setTeacherData] = useState([]);
  ...
  const fetchData = async () => {
    try {
      ...
      if (response.data) {
        setTeacherData(response.data.data.TeacherApprove);
                                    ...
                                    <tbody>
                                        {
                                            teacherData && teacherData?.map((item, index) => (                                                
                                                <tr  key={index} className="border-t hover:bg-[rgba(204,40,40,0.1)] border-[rgba(204,40,40,0.2)]">                                                    
                                                    <td className="px-3 lg:px-4 py-2 lg:py-3 text-black text-sm lg:text-base font-medium font-inter text-left">{item?.userId?.name || 'No Name'}</td>
                                                    <td className="px-3 lg:px-4 py-2 lg:py-3 text-black text-sm lg:text-base font-medium font-inter text-left">{item?.userId?.email || 'No Name'}</td>
                                                    <td className="px-3 lg:px-4 py-2 lg:py-3 text-black text-sm lg:text-base font-medium font-inter">{item?.city || 'N/A'}</td>
                                                    <td className="px-3 lg:px-4 py-2 lg:py-3 text-black text-sm lg:text-base font-medium font-inter"> {item?.experience || 'N/A'} years</td>
