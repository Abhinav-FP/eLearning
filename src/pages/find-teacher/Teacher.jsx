                        ...
                        teacher && teacher?.map((item, i) => (
                            <Link href={`/teacher/${item?._id}`}  key={i} className="w-full sm:w-6/12 md:w-4/12 lg:w-3/12 px-2.5 mb-5" >
                                <div className="bg-[#FFE8E8] rounded-[10px] px-4 lg:px-6 py-5 lg:py-[30px]">
                                    ...
                                    <p className="text-[#535353] text-base font-medium -tracking-[0.03em] text-left mb-4 lg:mb-5 line-clamp-2">{item.description}</p>
                                    <div className="flex flex-wrap gap-3 justify-center">
                                        <span className="text-[#CC2828] -tracking-[0.03em] font-bold text-base">${item.average_price}/{item?.average_duration} min</span>
                                    </div>
                                </div>
                            </Link>
                        ))
